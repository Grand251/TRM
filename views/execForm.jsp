	...
	<section>
		<c:forEach items="${trainings}" var="training">
			<article>
				...
				<form action="execdashboard/${training.executiveWorkflowStatusId}">
					<p>
						1) Send Email <input type="checkbox" name="email"
							<c:if 
								test="${training.invitationsSent == '1'}">checked="checked"
							</c:if> />
					</p>
					<p>
						2) Skill Enrollment <input type="checkbox" name="enrollment"
							<c:if test="${training.skillportEnrollmentsCompleted == '1'}">checked="checked"</c:if> />
					</p>
					<p>
						3) Assessment Recorded <input type="checkbox" name="assessment"
							<c:if test="${training.assessmentsRecorded == '1'}">checked="checked"</c:if> />
					</p>
					<p>
						4) Vendor Training Security Clearance <input type="checkbox"
							name="vendor"
							<c:if test="${training.vendorTrainingClearance == '1'}">checked="checked"</c:if> />
					</p>
					<p>
						5) Feedback Completed <input type="checkbox" name="feedback"
							<c:if test="${training.feedbackCompleted == '1'}">checked="checked"</c:if> />
					</p>
					<p>
						6) Send Training Completion <input type="checkbox"
							name="completion"
